...
const Quotes = () => {
  ...
  React.useEffect(() => {
    const initSlider = () => {
      if (typeof window !== 'undefined' && window.M && sliderRef.current) {
        const sliderElement = sliderRef.current.querySelector('.slider');
        if (sliderElement) {
          const sliderInstance = window.M.Slider.init(sliderElement, {
            indicators: true,
            duration: 500,
          });

          // Store instance for cleanup
          sliderElement._sliderInstance = sliderInstance;
        }
      }
    };
    ...
    return () => {
      ...
      if (sliderRef.current) {
        const sliderElement = sliderRef.current.querySelector('.slider');
        if (sliderElement && sliderElement._sliderInstance) {
          sliderElement._sliderInstance.destroy();
        }
      }
    };
  }, []);
  ...
  const quotes = [
    ...
    return (
      <div className="quotes-container color-schema-light default-padding-top-bottom" ref={sliderRef}>
        <div className="row">
